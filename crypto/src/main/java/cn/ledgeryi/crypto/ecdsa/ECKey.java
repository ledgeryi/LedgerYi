package cn.ledgeryi.crypto.ecdsa;
...
public class ECKey implements Serializable, SignInterface {
  ...
  public ECKey(SecureRandom secureRandom) {
    this(LedgerYiCastleProvider.getInstance(), secureRandom);
  }
  ...
  public ECKey(byte[] key, boolean isPrivateKey) {
    ...
    this.provider = LedgerYiCastleProvider.getInstance();
  }
  ...
  public ECKey(@Nullable BigInteger priv, ECPoint pub) {
    this(
        LedgerYiCastleProvider.getInstance(),
  ...
  private static PrivateKey privateKeyFromBigInteger(BigInteger priv) {
    if (priv == null) {
      ...
      try {
        return ECKeyFactory
            .getInstance(LedgerYiCastleProvider.getInstance())
  ...
  public ECKey(BigInteger priv, byte[] publicKey){
    this(priv,CURVE.getCurve().decodePoint(publicKey));
  }
  ...
  public static boolean verify(byte[] data, ECDSASignature signature, byte[] pub) {
    ...
    try {
      ...
      // Bouncy Castle contains a bu3g that can cause NPEs given
  ...
  public boolean verify(byte[] data, ByteString signature){
    return ECKey.verify(data,getECDSASignature(signature), getPubKey());
  }
  ...
  private ECKey.ECDSASignature getECDSASignature(ByteString sign){
    ...
    byte[] signatureEncoded = Base64.decode(signature.toBase64());
    return ECKey.ECDSASignature.fromComponents(Arrays.copyOfRange(signatureEncoded, 1, 33),
