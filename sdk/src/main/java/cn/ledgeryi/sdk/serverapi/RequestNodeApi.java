...
public class RequestNodeApi {
    ...
    public boolean clearContractABI(byte[] owner, byte[] contractAddress, byte[] privateKeys) throws IOException, CipherException {
        if (owner == null) {
            owner = getAddress();
        }
    ...
    public boolean triggerContract(byte[] owner, byte[] contractAddress, long callValue,
                                   byte[] data, long feeLimit, long tokenValue, String tokenId, boolean isConstant, byte[] privateKey)
        ...
        if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
            log.error("RPC create call trx failed!");
        ...
        // for constant
        if (transaction.getRetCount() != 0 && transactionExtention.getConstantResult(0) != null && transactionExtention.getResult() != null) {
    ...
    public boolean deployContract(
            ...
            long tokenValue,
            String tokenId, byte[] privateKey) {
        if (owner == null) {
            owner = getAddress();
        }
    ...
    private TriggerSmartContract triggerCallContract(
            byte[] address,
    ...
    private ClearABIContract createClearABIContract(byte[] owner, byte[] contractAddress) {
    ...
    private static boolean processTransaction(TransactionExtention transactionExtention, byte[] privKeyBytes) {
        ...
        log.info("before sign transaction hex string is " + ByteArray.toHexString(transaction.toByteArray()));
        transaction = TransactionUtils.sign(transaction, privKeyBytes);
        return rpcCli.broadcastTransaction(transaction);
    }

    private CreateSmartContract createContractDeployContract(
            String contractName,
            byte[] address,
            String ABI,
            String code,
