...
import cn.ledgeryi.framework.common.utils.Value;
...
public class RLP {
  ...
  private static final Logger log = LoggerFactory.getLogger("rlp");
  ...
  static void fullTraverse(byte[] msgData, int level, int startPos,
    ...
    try {
      ...
      while (pos < endPos) {
        ...
        log.error("fullTraverse: level: " + level + " startPos: " + pos + " endPos: " + endPos);
  ...
  public static DecodeResult decode(byte[] data, int pos) {
    ...
    if (prefix == OFFSET_SHORT_ITEM) {  // 0x80
      ...
      int lenbytes = ByteUtil.byteArrayToInt(
      ...
      int lenlist = ByteUtil.byteArrayToInt(
          copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length of encoded bytes
