  ...
  public class DisConnectScore extends Score<NodeStatistics> {
    ...
    int calculate(int baseScore) {
      if (t.wasDisconnected()) {
        if (t.getLedgerYiLastLocalDisconnectReason() == null
            && t.getLedgerYiLastRemoteDisconnectReason() == null) {
          // means connection was dropped without reporting any reason - bad
          baseScore *= 0.8;
        } else if (t.getLedgerYiLastLocalDisconnectReason() != ReasonCode.REQUESTED) {
          // the disconnect was not initiated by discover mode
          if (t.getLedgerYiLastRemoteDisconnectReason() == ReasonCode.TOO_MANY_PEERS
              || t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.TOO_MANY_PEERS
              || t.getLedgerYiLastRemoteDisconnectReason() == ReasonCode.TOO_MANY_PEERS_WITH_SAME_IP
              || t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.TOO_MANY_PEERS_WITH_SAME_IP
              || t.getLedgerYiLastRemoteDisconnectReason() == ReasonCode.DUPLICATE_PEER
              || t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.DUPLICATE_PEER
              || t.getLedgerYiLastRemoteDisconnectReason() == ReasonCode.TIME_OUT
              || t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.TIME_OUT
              || t.getLedgerYiLastRemoteDisconnectReason() == ReasonCode.PING_TIMEOUT
              || t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.PING_TIMEOUT
              || t.getLedgerYiLastRemoteDisconnectReason() == ReasonCode.CONNECT_FAIL
              || t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.CONNECT_FAIL) {
            // The peer is popular, but we were unlucky
            baseScore *= 0.9;
          } else if (t.getLedgerYiLastLocalDisconnectReason() == ReasonCode.RESET) {
            baseScore *= 0.95;
          } else if (t.getLedgerYiLastRemoteDisconnectReason() != ReasonCode.REQUESTED) {
