...
import static cn.ledgeryi.framework.common.overlay.message.StaticMessages.PING_MESSAGE;
import static cn.ledgeryi.framework.common.overlay.message.StaticMessages.PONG_MESSAGE;
...
import cn.ledgeryi.framework.common.overlay.discover.node.statistics.MessageStatistics;
...
public class P2pHandler extends SimpleChannelInboundHandler<P2pMessage> {
  ...
  private final static ScheduledExecutorService pingTimer =
  ...
  public void channelRead0(final ChannelHandlerContext ctx, P2pMessage msg) {
    ...
    switch (msg.getType()) {
      case P2P_PING:
        ...
        if (count > 3) {
          log.warn("TCP attack found: {} with ping count({})", ctx.channel().remoteAddress(), count);
      ...
      case P2P_PONG:
        if (messageStatistics.p2pInPong.getTotalCount() > messageStatistics.p2pOutPing.getTotalCount()) {
          log.warn("TCP attack found: {} with ping count({}), pong count({})",
